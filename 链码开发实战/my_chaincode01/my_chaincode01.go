 ...
 */
 ...
 func (t *SmartContract) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
  ...
  func (t *SmartContract) initDepartment(stub shim.ChaincodeStubInterface, args []string) pb.Response {
    ...
    // 创建联合主键，可以通过主键"department+id"（如"department1"）找到对应部门
 ...
 func (t *SmartContract) addEmployee(stub shim.ChaincodeStubInterface, args []string) pb.Response {
   if len(args) != 4 {
     return shim.Error("Incorrect number of arguments. Expecting 4 like (employeeId, employeeName, departmentId, jobs)")
   }

   employee, err := translateEmployeeFromBytes(args)
	 if err != nil {
	  	return shim.Error(err.Error())
	 }
 ...
 func queryAllDepartmentIDs(stub shim.ChaincodeStubInterface) []string {
   // 复合键查询
	compositeKeys, err := stub.GetStateByPartialCompositeKey("Department", []string{"department"})
	if err != nil {
		return nil
	}
	defer compositeKeys.Close()

	departmentIds := make([]string, 0)
	for i := 0; compositeKeys.HasNext(); i++ {
		compositeKey, err := compositeKeys.Next()
		if err != nil {
			return nil
		}
 ...
 // 将Bytes类型数据转换为结构体
 func translateEmployeeFromBytes(args []string) (*Employee, error) {
   if len(args) != 4 {
